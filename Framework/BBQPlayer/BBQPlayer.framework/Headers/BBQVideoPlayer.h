...
// 创建
//- (BBQVideoPlayer *)player {
//
//    if (!_player) {
//
//        _player = [BBQVideoPlayer playerWithFrame:CGRectZero andDelegate:self];;
//    }
//    return _player;
//}
...
@class BBQVideoPlayer;

@protocol BBQVideoPlayerDelegate <NSObject>
// 更多按钮事件 代理
- (void)onVideoPlayer:(BBQVideoPlayer *)player andMoreBtn:(UIButton *)moreBtn;
// 关闭按钮事件 代理
- (void)onVideoPlayer:(BBQVideoPlayer *)player andCloseBtn:(UIButton *)closeBtn;

@end

NS_SWIFT_NAME(BBQPlayer)
@interface BBQVideoPlayer : UIView
// 创建 video player 对象。 内部做了4g 能否播放的处理 、加载视频失败的处理 视频全屏 都处理好了

+ (instancetype)playerWithFrame:(CGRect)frame andDelegate:(id <BBQVideoPlayerDelegate>) mdelegate;
