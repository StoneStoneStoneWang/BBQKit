...
extension BBQApi: WLObserverReq {
    ...
    public var params: Dictionary<String, Any> {
        ...
        case let .removeBlack(encoded): return ["projectId":BBQConfigure.fetchAppKey(),"bt.encoded": encoded]
            
        case let .focus(OUsEncoded, targetEncoded: targetEncoded): return ["atn.OUsEncoded":OUsEncoded,"atn.targetEncoded":targetEncoded,"projectId": BBQConfigure.fetchAppKey(),"atn.tableName":"Users"]
        case let .fetchMyFocus(page): return ["page":page,"projectId": BBQConfigure.fetchAppKey()]
            
        case let .publish(tag,content: content): return ["cfs.tag":tag,"cfs.projectId":BBQConfigure.fetchAppKey(),"cfs.content":content]
            
        case let .fetchList(tag, page: page): return ["cfs.tag":tag,"cfs.projectId":BBQConfigure.fetchAppKey(),"page":page]
            
        case let .fetchMyList(tag, page: page): return ["cfs.tag":tag,"cfs.projectId":BBQConfigure.fetchAppKey(),"page":page]
            
        case let .fetchComments(page, targetEncoded: targetEncoded):
            
            return ["comment.targetEncoded":targetEncoded,"page":page,"limit": 20,"projectId": BBQConfigure.fetchAppKey()]
            
        case let .addComment(targetEncoded, content: content, tablename: tablename, type: type):
            
            return ["comment.targetEncoded":targetEncoded,"comment.content":content,"comment.tableName": tablename,"comment.type": type,"projectId": BBQConfigure.fetchAppKey()]
        ...
        case let .report(OUsEncoded, targetEncoded: targetEncoded, type: type, content: content):
            ...
            return ["rt.OUsEncoded":OUsEncoded,"rt.targetEncoded":targetEncoded,"rt.type": type,"rt.content": content,"rt.projectId": BBQConfigure.fetchAppKey(),"rt.tableName": "CircleFriends"]
        case let .addBlack(OUsEncoded, targetEncoded: targetEncoded, content: content):
            
            return ["bt.OUsEncoded":OUsEncoded,"bt.targetEncoded":targetEncoded,"bt.content": content,"projectId": BBQConfigure.fetchAppKey(),"bt.tableName": "CircleFriends"]
            
        case let .attention(OUsEncoded, targetEncoded: targetEncoded): return ["atn.OUsEncoded":OUsEncoded,"atn.targetEncoded":targetEncoded,"projectId": BBQConfigure.fetchAppKey(),"atn.tableName":"Users"]
            
        case let .fetchPublish(tag, page: page, userId: userId): return ["cfs.projectId":BBQConfigure.fetchAppKey(),"cfs.tag": tag,"page": page,"userId": userId]
            
        case .fetchAddress: return [:]
            
        case let .deleteAddress(encode): return ["pe.encoded":encode]
            
        case .fetchBanners: return ["cfs.tag":"","cfs.projectId":BBQConfigure.fetchAppKey(),"page":1]
        ...
        case .deleteMyCircle(let encode): return ["cfs.encoded": encode,"cfs.projectId":BBQConfigure.fetchAppKey()]
    ...
    public var host: String {
        ...
        default: return BBQConfigure.fetchDomain()
