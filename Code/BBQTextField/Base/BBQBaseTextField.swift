...
fileprivate typealias WLTextChanged = (BBQBaseTextField) -> ()

@objc (BBQBaseTextField)
open class BBQBaseTextField: UITextField {
    ...
    // MARK: 编辑类型 详情参考 枚举
    fileprivate var editType: BBQTextFiledEditType = .phone {
...
/** 文本框内容 样式 */
extension BBQBaseTextField {
    @objc (BBQTextFiledEditType)
    public enum BBQTextFiledEditType: Int {
        @objc (BBQTextFiledEditTypepriceEdit)
        case priceEdit
...
extension BBQBaseTextField {
    ...
    open func makeAttribute(_ closure: @escaping (BBQBaseTextField) -> ()) {
...
extension BBQBaseTextField {
    ...
    @objc (set_editType:)
    public func set_editType(_ editType: BBQTextFiledEditType) {
    ...
    @objc (set_textChanged:)
    public func set_textChanged(_ textChanged: @escaping (BBQBaseTextField) -> ()) {
        
        self.textChanged = textChanged
    }
    @objc (set_topLineFrame:)
    public func set_topLineFrame(_ frame: CGRect) {
        
        topLineFrame = frame
    }
    @objc (set_bottomLineFrame:)
    public func set_bottomLineFrame(_ frame: CGRect) {
        
        bottomLineFrame = frame
    }
    @objc (set_topLineColor:)
    public func set_topLineColor(_ color: UIColor) {
        
        topLineColor = color
    }
    @objc (set_bottomLineColor:)
    public func set_bottomLineColor(_ color: UIColor) {
        
        bottomLineColor = color
    }
    @objc (set_secureTextEntry:)
    public func set_secureTextEntry(_ isSecureTextEntry: Bool) {
...
extension BBQBaseTextField {
    ...
    open override func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        return __shouldChangeCharacters(target: textField as! BBQBaseTextField,range: range,string: string)
    }
    
    private func __shouldChangeCharacters(target: BBQBaseTextField , range: NSRange, string: String) -> Bool {
...
extension BBQBaseTextField {
    ...
    @objc open func textFieldDidChange(_ textField: BBQBaseTextField) {
        
        __textDidChange(target: textField)
    }
    
    // MARK: editChanged
    private func __textDidChange(target: BBQBaseTextField) {
