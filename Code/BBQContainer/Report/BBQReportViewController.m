...
@import Masonry;
@import SToolsKit;
@interface BBQReportTableViewCell()
...
@implementation BBQReportTableViewCell
...
- (UIButton *)selectItem {
    ...
    if (!_selectItem) {
        ...
        [_selectItem setImage:[UIImage imageNamed:@BBQNormalIcon] forState:UIControlStateNormal];
        
        [_selectItem setImage:[UIImage imageNamed:@BBQSelectedIcon] forState:UIControlStateSelected];
...
- (void)setReportBean:(BBQReportBean *)reportBean {
...
@interface BBQReportViewController ()

@property (nonatomic ,strong) BBQReportBridge *bridge;
...
@property (nonatomic ,copy) BBQReportBlock block;
@end

@implementation BBQReportViewController
...
- (instancetype)initWithUid:(NSString *)uid andEncode:(NSString *)encode andBlock:( BBQReportBlock)block{
...
- (void)configOwnSubViews {
    ...
    [self.tableView registerClass:[BBQReportTableViewCell class] forCellReuseIdentifier:@"cell"];
...
- (UITableViewCell *)configTableViewCell:(id)data forIndexPath:(NSIndexPath *)ip {
    ...
    BBQReportTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"cell"];
...
- (void)configViewModel {
    ...
    self.bridge = [BBQReportBridge new];

    __weak typeof(self) weakSelf = self;
    
    [self.bridge createReport:self reports:BBQReportKeyValues uid:self.uid encoded:self.encode textView:self.textView reportAction:^{
